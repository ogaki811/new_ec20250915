...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initPaymentMethods() {
        ...
        const codFeeRow = document.querySelector('.cod-fee');
        ...
        paymentRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                // Show relevant section and update pricing
                switch (method) {
                    case 'credit':
                        if (creditCardForm) {
        ...
        function updateRequiredFields(creditCardRequired) {
            const cardFields = ['cardNumber', 'expiryMonth', 'expiryYear', 'cvv', 'cardName'];
            cardFields.forEach(fieldId => {
                const field = document.getElementById(fieldId);
                if (field) {
                    field.required = creditCardRequired;
                }
            });
        }
    }
    ...
    function initCardFormatting() {
        ...
        if (cardNumberInput) {
            cardNumberInput.addEventListener('input', function() {
                ...
                // Detect card type and show appropriate brand
    ...
    function detectCardType(number) {
        const cardBrands = document.querySelector('.card-brands');
        if (!cardBrands) return;
        
        // Reset all brand styles
        const brandImages = cardBrands.querySelectorAll('img');
        ...
        if (number.startsWith('4')) {
            // VISA
            brandImages[0].style.opacity = '1';
        } else if (number.startsWith('5') || number.startsWith('2')) {
            // MasterCard
            brandImages[1].style.opacity = '1';
        }
    }
    
    // Initialize receipt toggle
    function initReceiptToggle() {
        const needReceiptCheckbox = document.getElementById('needReceipt');
        const receiptDetails = document.getElementById('receiptDetails');
        
        if (needReceiptCheckbox && receiptDetails) {
            needReceiptCheckbox.addEventListener('change', function() {
                if (this.checked) {
                    receiptDetails.style.display = 'block';
                    setTimeout(() => {
                        receiptDetails.style.opacity = '1';
                        receiptDetails.style.transform = 'translateY(0)';
                    }, 10);
                } else {
                    receiptDetails.style.opacity = '0';
                    receiptDetails.style.transform = 'translateY(-10px)';
                    setTimeout(() => {
            ...
            // Set initial styles
            receiptDetails.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
            receiptDetails.style.opacity = '0';
            receiptDetails.style.transform = 'translateY(-10px)';
        }
    }
    ...
    function validateCardNumber(field) {
        ...
        if (value.length < 13 || value.length > 19) {
            field.classList.add('error');
            showFieldError(field, '有効なカード番号を入力してください');
            return false;
        }
        ...
        if (!luhnCheck(value)) {
            field.classList.add('error');
            showFieldError(field, '無効なカード番号です');
            return false;
        }
    ...
    // Initialize delivery time update
    function initDeliveryTimeUpdate() {
        const deliveryTimeRadios = document.querySelectorAll('input[name="deliveryTime"]');
        const deliveryTimeDisplay = document.querySelector('.delivery-time');
        ...
        deliveryTimeRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                const timeLabels = {
                    ...
                    'afternoon3': '16:00-18:00',
                    'evening': '18:00-20:00'
                };
                
                if (deliveryTimeDisplay) {
                    deliveryTimeDisplay.textContent = `時間指定: ${timeLabels[this.value] || '指定なし'}`;
                }
            });
        });
    }
    ...
    function updateOrderSummary() {
        orderSummary.total = orderSummary.subtotal + orderSummary.shipping + orderSummary.codFee - orderSummary.discount;
        
        // Update total display
        const totalElement = document.querySelector('.summary-row.total .value');
        if (totalElement) {
            totalElement.textContent = `¥${orderSummary.total.toLocaleString()}`;
        }
        
        // Update COD fee display
        const codFeeElement = document.querySelector('.cod-fee .value');
        if (codFeeElement) {
            codFeeElement.textContent = `¥${orderSummary.codFee.toLocaleString()}`;
        }
    }
    ...
    function initFormSubmission() {
        ...
        if (agreeTermsCheckbox && placeOrderBtn) {
    ...
    function validateForm() {
        ...
        if (agreeTerms && !agreeTerms.checked) {
            showNotification('利用規約に同意してください', 'warning');
            agreeTerms.focus();
            isValid = false;
        }
