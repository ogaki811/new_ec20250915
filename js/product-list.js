...
document.addEventListener('DOMContentLoaded', function() {
        ...
        addToCartBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                setTimeout(() => {
                    ...
                    // Show notification
                    if (window.ECUtils) {
                        window.ECUtils.showNotification(`${productTitle}をカートに追加しました`, 'success');
                    }
        ...
        favoriteBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (isActive) {
                    ...
                    if (window.ECUtils) {
                        window.ECUtils.showNotification(`${productTitle}をお気に入りから削除しました`, 'info');
                    }
                } else {
                    // Add to favorites
                    this.classList.add('active');
                    ...
                    if (window.ECUtils) {
                        window.ECUtils.showNotification(`${productTitle}をお気に入りに追加しました`, 'success');
                    }
                }
            });
        });
    ...
    function updateActiveFilters() {
        ...
        if (currentFilters.price.min > 0 || currentFilters.price.max < 50000) {
            addFilterTag(`価格: ¥${currentFilters.price.min.toLocaleString()} - ¥${currentFilters.price.max.toLocaleString()}`, 'price');
        }
        ...
        Object.keys(currentFilters).forEach(filterType => {
            ...
            if (Array.isArray(filterValues) && filterValues.length > 0) {
                filterValues.forEach(value => {
                    addFilterTag(`${getFilterTypeLabel(filterType)}: ${value}`, filterType, value);
                });
            }
        });
        
        function addFilterTag(label, filterType, filterValue = null) {
            const tag = document.createElement('div');
            tag.className = 'filter-tag';
            tag.innerHTML = `
                <span>${label}</span>
                <button class="remove-filter" data-filter-type="${filterType}" data-filter-value="${filterValue || ''}">
                    ×
                </button>
            `;
    ...
    function removeFilter(filterType, filterValue) {
        if (filterType === 'price') {
            currentFilters.price = { min: 0, max: 50000 };
            const minInput = document.getElementById('priceMin');
            const maxInput = document.getElementById('priceMax');
            if (minInput) minInput.value = 0;
            if (maxInput) maxInput.value = 50000;
            ...
            const checkbox = document.querySelector(`input[data-filter-type="${filterType}"][value="${filterValue}"]`);
            if (checkbox) {
                checkbox.checked = false;
            }
        }
    ...
    function updateProductGrid() {
        // This would typically fetch and render new products
    ...
    function updateResultsCount() {
        ...
        if (resultsCount) {
            ...
            resultsCount.innerHTML = `
                <span class="highlight">${totalResults}</span>件の商品が見つかりました
                （<span class="highlight">${startItem}</span>-<span class="highlight">${endItem}</span>件目を表示）
            `;
        }
    }
    ...
    function updatePagination() {
        const totalResults = 247; // Example total
        ...
        if (paginationInfo) {
            const startItem = (currentPage - 1) * itemsPerPage + 1;
            const endItem = Math.min(currentPage * itemsPerPage, totalResults);
            paginationInfo.textContent = `${startItem}-${endItem}件目 / ${totalResults}件中`;
        }
    }
    ...
    function hideLoadingState() {
        ...
        if (productsContainer) {
            ...
            // This would typically render the actual products
            productsContainer.innerHTML = `
    ...
    function openQuickView(productId) {
        ...
        // Load product data (simulate)
        setTimeout(() => {
            const loading = modal.querySelector('.quick-view-loading');
            loading.innerHTML = `
                <h2>商品クイックビュー</h2>
                <p>商品ID: ${productId}の詳細情報がここに表示されます。</p>
                <button onclick="window.location.href='product-detail.html'" style="
