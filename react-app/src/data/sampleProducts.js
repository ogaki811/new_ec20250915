...
const getProductImages = (index) => {
  // 画像の枚数をランダムに決定（1-4枚）
  const imageCount = (index % 4) + 1; // 1, 2, 3, 4のいずれか
  const images = [];
...
export const categories = ['文具・事務用品', '家具', '電化製品', '収納用品'];
export const brands = ['コクヨ', 'オカムラ', 'ゼブラ', 'プラス', 'ぺんてる', 'ロジクール', 'パナソニック', 'アイリスオーヤマ', 'ニトリ', 'カシオ', 'オルファ', 'スリーエム', 'トンボ鉛筆', 'エルゴトロン', 'エレコム', 'サンワサプライ', 'エーワン', 'マックス'];
